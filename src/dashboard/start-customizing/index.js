...
const StartCustomizing = () => {
		...
		return ( <
			> {
				...
				<
				h2 > {
					__('Start Customizing', 'generatepress')
				} < /h2> <
				/div>
				...
				div className = "generatepress-dashboard__section" > {
					Object.keys(customizeItems).map((item, index) => {
							return ( <
								div className = "generatepress-dashboard__section-item"
								key = {
									index
								} >
								<
								...
								{
									!!customizeItems[item].description && < div className = "generatepress-dashboard__section-item-description" > {
											customizeItems[item].description
										} < /div> } <
										/div>
