...
const GeneratePressColorManagerControlForm = (props) => {
	...
	return ( <
		...
		div className = "generate-component-color-picker-wrapper generate-color-manager-wrapper" > {
			colors.map((color, index) => {
				...
				return ( <
					...
					<
					Tooltip text = {
						__('Delete Color', 'generatepress')
					} >
					<
					Button className = "generate-color-manager--delete-color"
					onClick = {
						() => {
		...
		<
		Tooltip text = {
			__('Add Global Color', 'generatepress')
		} >
		<
		Button className = "generate-color-manager--add-color"
		onClick = {
			() => {
