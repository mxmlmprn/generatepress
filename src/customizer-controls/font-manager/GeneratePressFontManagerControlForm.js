...
const GeneratePressFontManagerControlForm = (props) => {
	...
	const fontFamilies = [{
		...
		},
		{
			...
			options: Object.keys(googleFonts).map((font) => ({
				value: font,
				label: font
			})),
		},
...

		{
			!!fonts.length > 0 && fonts.map((font, index) => {
				...
				return ( <
					...
					<
					Button className = "generate-font-manager--label"
					onClick = {
						() => {
							if (itemId !== isOpen) {
								setOpen(itemId);
					...
					<
					Tooltip text = {
						__('Open Font Family Settings', 'generatepress')
					} >
					<
					Button className = "generate-font-manager--open"
					onClick = {
						() => {
							if (itemId !== isOpen) {
								setOpen(itemId);
					...
					<
					Tooltip text = {
						__('Delete Font Family', 'generatepress')
					} >
					<
					Button className = "generate-font-manager--delete-font"
					onClick = {
						() => {
							// eslint-disable-next-line
...

					{
						...
						label = {
							__('Font family name', 'generatepress')
						}
						value = {
							currentFontFamily
						}
...

						{
...

							{
								!!fonts[index].googleFont &&
									...
									TextControl
								label = {
									__('Category', 'generatepress')
								}
								value = {
									fonts[index].googleFontCategory || ''
								}
								onChange = {
									(value) => {
										const fontValues = [...fonts];
								...
								TextControl
								label = {
									__('Variants', 'generatepress')
								}
								value = {
									fonts[index].googleFontVariants || ''
								}
								onChange = {
									(value) => {
										const fontValues = [...fonts];
