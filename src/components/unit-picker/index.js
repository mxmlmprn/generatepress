...
const GeneratePressUnitPickerControl = (props) => {
	...
	return ( <
		...
		<
		Dropdown className = "generate-component-control--unit-picker"
		...
		renderContent = {
			({
				...
				return ( <
					ButtonGroup className = "components-generate--control-buttons__units"
					aria - label = {
						__('Select Units', 'generatepress')
					} > {
						units.map((unit) => {
							let unitName = unit;
							...
							text = {
								!!unit ? sprintf(__('%s Units', 'generatepress'), unitName) : unitName
							}
							key = {
									unit
								} >
							...
							className = {
								'components-generate--control-button__units--' + unit
							}
							isSmall
							isPrimary = {
								value === unit
