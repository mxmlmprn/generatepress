...
const GeneratePressColorPickerControl = (props) => {
	...
	return ( <
...

		{
			...
			label = {
				!!props.label && !hideLabel ? props.label : ''
			}
			...
			div className = "generate-color-option-area" > {
					...
					TextControl
					label = {
						__('CSS Variable Name', 'generatepress')
					}
					disabled = {
						!!isVarLock
					}
...

					{
						!!isVarLock &&
							<
							Tooltip text = {
								__('Changing this name will remove its color from elements already using it.', 'generatepress')
							} >
							<
							Button
						onClick = {
