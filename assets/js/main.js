(function() {
	...
	if ("querySelector" in document && "addEventListener" in window) {
		...
		var allNavToggles = document.querySelectorAll(".menu-toggle"),
			...
			mobileMenuControls = document.querySelector(
		...
		var enableDropdownArrows = function(nav) {
			if (body.classList.contains("dropdown-hover")) {
				...
				for (var i = 0; i < dropdownItems.length; i++) {
					...
					dropdownItems[i]
						.querySelector(".dropdown-menu-toggle")
						.setAttribute("aria-label", generatepressMenu.openSubMenuLabel);
				}
			}
		};
		...
		var setDropdownArrowAttributes = function(arrow) {
			if (
				"false" === arrow.getAttribute("aria-expanded") ||
				!arrow.getAttribute("aria-expanded")
			) {
		...
		var toggleNav = function(e, _this) {
			...
			if (_this.getAttribute("data-nav")) {
				parentContainer = document.getElementById(
					_this.getAttribute("data-nav")
				);
			} else {
				parentContainer = document.getElementById(
					_this.closest("nav").getAttribute("id")
				);
			}
			...
			if (parentContainer.classList.contains("toggled")) {
				...
				if (isExternalToggle) {
					...
					mobileMenuControls &&
					parentContainer.classList.contains("main-navigation")
				) {
					mobileMenuControls.classList.remove("toggled");
				}
		...
		var toggleSubNav = function(e, _this) {
			...
			if (
				(_this.closest("nav").classList.contains("toggled") ||
					htmlEl.classList.contains("slide-opened")) &&
				!body.classList.contains("dropdown-click")
			) {
				e.preventDefault();
				var closestLi = _this.closest("li");

				setDropdownArrowAttributes(
					closestLi.querySelector(".dropdown-menu-toggle")
				);
		...
		var checkMobile = function() {
			var openedMobileMenus = document.querySelectorAll(
				".toggled, .has-active-search"
			);
			...
			for (var i = 0; i < openedMobileMenus.length; i++) {
				...
				if (menuToggle && menuToggle.offsetParent === null) {
					if (openedMobileMenus[i].classList.contains("toggled")) {
						...
						if (
							openedMobileMenus[i].classList.contains(
								"mobile-menu-control-wrapper"
							)
						) {
							remoteNav = true;
						...
						if (!remoteNav) {
							...
							var closestNav = openedMobileMenus[i].getElementsByTagName(
									"ul"
								)[0],
			...
			for (var i = 0; i < navLinks.length; i++) {
				navLinks[i].addEventListener(
					...
					function(e) {
						...
						if (
							closest_nav.classList.contains("toggled") ||
							htmlEl.classList.contains("slide-opened")
						) {
	...
	if ("querySelector" in document && "addEventListener" in window) {
		...
		if (body.classList.contains("dropdown-hover")) {
			var navLinks = document.querySelectorAll("nav .main-nav ul a"),
				menuBarItems = document.querySelectorAll(
